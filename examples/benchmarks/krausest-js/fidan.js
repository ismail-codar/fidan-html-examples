var fidan = (function(exports) {
  ...
  var reuseNodes = function(
    ...
    for (
      var i$1 = 0, item = void 0, head = _head, mode = _mode;
      i$1 < data.length;
      i$1++
    ) {
      item = data[i$1];
  ...
  var reconcile = function(
    ...
    if (renderedValues.length === 0) {
      ...
      for (var i = 0, len = data.length; i < len; i++) {
        ...
        mode
          ? parent.insertBefore(node$1, afterNode)
    ...
    if (prevEnd < prevStart) {
      if (newStart <= newEnd) {
        ...
        while (newStart <= newEnd) {
          ...
          mode$1
            ? parent.insertBefore(node$2, afterNode)
            : parent.appendChild(node$2);
          newStart++;
        }
      }
    ...
    for (var i$1 = newStart; i$1 <= newEnd; i$1++) {
      P[i$1] = -1;
    } // Index to resolve position from current to new

    var I = new Map();

    for (var i$2 = newStart; i$2 <= newEnd; i$2++) {
      I.set(data[i$2], i$2);
    }
    ...
    if (reusingNodes === 0) {
      ...
      for (var i$4 = newStart; i$4 <= newEnd; i$4++) {
        ...
        mode$2
          ? parent.insertBefore(node$4, afterNode)
          : parent.appendChild(node$4);
      }
    ...
    for (var i$6 = 0; i$6 < toRemove.length; i$6++) {
      parent.removeChild(nodes[toRemove[i$6]]);
    }
  ...
  var arrayMap = function(arr, parentDom, renderReturn, renderMode) {
    ...
    var arrayComputeRenderAll = function(nextVal) {
      if (!renderMode) {
        ...
        if (
          firstRenderOnFragment === undefined &&
          nextVal &&
          nextVal.length > 0
        ) {
